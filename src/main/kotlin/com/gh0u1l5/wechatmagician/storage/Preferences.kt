...
class Preferences : SharedPreferences {
    ...
    // setCache caches the string lists in memory to speed up getStringList()
    private val setCacheLock = ReentrantReadWriteLock()
    private val setCache = mutableMapOf<String, MutableSet<String>>()
    ...
    fun cacheStringSet() {
        PREFERENCE_STRING_LIST_KEYS.forEach { key ->
            val set = getString(key, "").split(" ")
            setCacheLock.write {
                setCache[key] = set.toMutableSet()
            }
        }
    }
